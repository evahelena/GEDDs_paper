...
for k in range(1):
    ...
    for i in range(boots):
        ...
        nA = randomizeArr(y)
        Nregions.append(energy(nA))
    
    fig = plt.figure()    
    bins = plt.hist(Nregions, 80)
    plotFDR(bins)
    plt.axvline(energy(y), linewidth=2, color='g', label = 'original value')
    plt.legend()

    
    nameOffile = fileIN+'_chr'+chrnum+'_frac'+str(f)+'-'+str(energy(y))+'_'+str(boots)+'boots_energy.png'
